...
module.exports = {
    ...
    async consultar_trazabilidad_reg_indicador(id_indicador, profesional, vigencia, periodo) {
        const resultados = await conexion.query("select r.*, cal.estado, pm.nombre_mes from schema_control.registroindicadores r inner join schema_control.calificacion_registro_indicadores as cal on (cal.id_registro_indicador = r.id_registroindicador) inner join schema_control.periodo_mes pm on(r.periodoevaluado = pm.id_mes) where r.id_indicador = $1 and r.id_profesional = $2 and r.vigencia = $3 and r.periodoevaluado = $4", [id_indicador, profesional, vigencia, periodo]);
        return resultados.rows;
    },
