...
router.get(
  "/obtener_soportesxreg_indicador/:id/:tipo",
  authMiddleware,
  (req, res) => {
    ...
    modelControlMando
      .consultar_soportes_x_regIndicador(req.params.id)
      .then((lista_soportes) => {
        //console.log(lista_soportes);
        var bandera_detalle = false;
        
        if (req.params.tipo == "detalle") {
          bandera_detalle =true;
        }

        res.render(config.rutaPartials + "soporte/list", {
          layout: false,
          lista_soportes: lista_soportes,
          id_reg_indicador: req.params.id,
          es_detalle: bandera_detalle,
        });
      });
  }
);
  ...
  modelControlMando
    ...
    .then((lista_soportes) => {
      ...
      var file = fs.readFileSync(
        path.join(config.rutaSoportesCtm, lista_soportes[0].nombre_soporte),
        "binary"
      );
