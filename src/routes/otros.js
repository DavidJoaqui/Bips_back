  ...
  modelControlMando
    ...
    .then((respuesta) => {
      if (respuesta["command"] == "INSERT" && respuesta["rowCount"] > 0) {
        ...
        res.json({
          status: 300,
          msg:
            "ERROR al crear el plan General" +
            req.query.nombre_plan +
            ", intente de nuevo...",
        });
      }
    ...
    .catch((err) => {
      ...
      res.json({
        status: 500,
        msg:
          "ERROR!! al almacenar el plan General " +
          req.query.nombre_plan +
          "Ocurrio un problema",
      });
    });
...
router.get("/ctm-lineasAccion", authMiddleware, (req, res) => {
  modelControlMando.consultar_RegistrosPlan_General().then((lista_planes) => {
    //console.log(lista_planes);
    res.render("paginas/lineasAccion", {
      user: req.session.user,
      listaPlanes_grales: lista_planes,
    });
  });
});

router.get(
  "/eliminar-popup-linea-accion/:id_linea/control-mando-bips",
  authMiddleware,
  (req, res) => {
    // console.log(req.query);
    //console.log(req.params);
    ...
    res.render(
      path.join(__dirname + "/src/vista/paginas/popup-eliminar-linea-accion"),
      { datos_linea: params }
    );
  }
);
    ...
    modelControlMando
      ...
      .then((respuesta) => {
        if (respuesta["command"] == "DELETE" && respuesta["rowCount"] > 0) {
          console.log(
            "respuesta de eliminacion: 1, Se elimino correctamente linea de accion..."
          );
          msg =
            "La linea de acción " +
            req.query.nombre_linea +
            " se eliminó correctamente...";
        } else {
          console.log(
            "respuesta de eliminacion: ERROR... 0, ocurrio un problema al eliminar la linea de acción " +
              req.query.nombre_linea
          );
          msg =
            " ocurrio un problema al eliminar la linea de acción... " +
            req.query.nombre_linea;
        }
...
router.get("/listado-ctm-lineas-accion", authMiddleware, (req, res) => {
  modelControlMando
    .consultar_RegistrosLineasAccion()
    .then((lista_lineas_accion) => {
      //console.log(lista_planes);
      res.render("paginas/lista_lineasAccion", {
        lista_lineas_accion: lista_lineas_accion,
      });
    });
});

router.post("/persistir-lineaAccion/", authMiddleware, (req, res) => {
  //res.send("OK");
  //console.log(req.query);
  //console.log(req.params);

  modelControlMando
    .insertar_lineaAccion(req.query.linea_accion, req.query.plan_general)
    .then((respuesta) => {
      if (respuesta["command"] == "INSERT" && respuesta["rowCount"] > 0) {
        ...
        res.json({
          status: 300,
          msg:
            "ERROR al crear la linea de Acción" +
            req.query.linea_accion +
            ", intente de nuevo...",
        });
      }
    ...
    .catch((err) => {
      ...
      res.json({
        status: 500,
        msg: "ERROR!! Ocurrio un problema al crear la linea de acción, contacte con el soporte...",
      });
    });
});
...
router.post("/form-editar-ctm-linea-accion", authMiddleware, (req, res) => {
  //res.send('OK');
  ...
  modelControlMando
    ...
    .then((listaPlanes_grales) => {
      //console.log(lista_planes);
      modelControlMando
        .consultar_LineasAccionXId(req.query.id_linea)
        .then((info_linea) => {
          res.render("paginas/editar_lineaAccion", {
            id_linea: req.query.id_linea,
...
router.post("/actualizar-linea-accion", authMiddleware, (req, res) => {
  //res.send("OK");
  console.log(req.query);
  console.log(req.params);
  ...
  modelControlMando
    ...
    .then((respuesta) => {
      ...
      if (respuesta["command"] == "UPDATE" && respuesta["rowCount"] > 0) {
        ...
        res.send({
          status: 300,
          msg:
            "ERROR al actualizar la linea de acción " +
            req.query.linea_accion +
            " intente de nuevo...",
        });
      }
    ...
    .catch((err) => {
      ...
      res.json({
        status: 500,
        msg:
          "ERROR!! La linea de acción " +
          req.query.linea_accion +
          ", " +
          +"NO se pudo actualizar, consulte a soporte...",
      });
    });
});
...
router.post("/persistir-objetivo/", authMiddleware, (req, res) => {
  ...
  modelControlMando
    .insertar_Objetivo(req.query.objetivo, req.query.linea_accion)
    .then((respuesta) => {
      if (respuesta["command"] == "INSERT" && respuesta["rowCount"] > 0) {
        console.log("OK... insert NEW entidad");
        //console.log(listaArchivos);
        ...
        res.json({
          status: 200,
          msg:
            "El objetivo " + req.query.objetivo + ", se creó correctamente...",
        });
      } else {
        //req.flash('error', 'ERROR al crear la entidad ' + req.query.nombre_entidad + ', con codigo ' + req.query.cod_entidad + ' intente de nuevo...');
        res.json({
...
router.get("/ctm-objetivos", authMiddleware, (req, res) => {
  modelControlMando
    ...
    .then((listaPlanes_grales) => {
      //console.log(lista_planes);
      res.render("paginas/ctm_objetivos", {
        user: req.session.user,
...
router.get(
  "/eliminar-popup-objetivo/:id_objetivo/control-mando-bips",
  authMiddleware,
  (req, res) => {
    // console.log(req.query);
    //console.log(req.params);
    ...
    res.render(
      path.join(__dirname + "/src/vista/paginas/popup-eliminar-objetivo"),
      { datos_objetivo: params }
    );
  }
);


router.get("/listado-ctm-objetivos", authMiddleware, (req, res) => {
  modelControlMando.consultar_RegistrosObjetivos().then((lista_objetivos) => {
    //console.log(lista_planes);
    res.render("paginas/lista_ctm_objetivos", {
...
router.post("/form-editar-ctm-objetivo", authMiddleware, (req, res) => {
  //res.send('OK');

  //consultar_RegistrosPlan_General_x_id

  modelControlMando
    .consultar_RegistroObjetivos_x_id(req.query.id_objetivo)
    .then((objetivo_info) => {
      modelControlMando
        .consultar_RegistrosPlan_General()
        .then((listaPlanes_grales) => {
          ...
          modelControlMando
            .consultar_RegistrosLineasAccion()
            .then((lineas_accion) => {
              res.render("paginas/editar_objetivo", {
                id_objetivo: req.query.id_objetivo,
                planes_generales: listaPlanes_grales,
    ...
    .then((respuesta) => {
      ...
      if (respuesta["command"] == "UPDATE" && respuesta["rowCount"] > 0) {
        ...
        res.send({
          status: 300,
          msg:
            "ERROR al actualizar el objetivo <b>" +
            req.query.objetivo +
            "</b> " +
            " intente de nuevo...",
        });
